...
# at = pd.read_csv("K:\project\phonepe project\Agg_Trans.csv")
# au = pd.read_csv("K:\project\phonepe project\Agg_User.csv")
# mt = pd.read_csv("K:\project\phonepe project\Map_Trans.csv")
# mu = pd.read_csv("K:\project\phonepe project\Map_User.csv")
# tt = pd.read_csv("K:\project\phonepe project\Top_Trans.csv")
# tu = pd.read_csv("K:\project\phonepe project\Top_User.csv")
...
with colT2:
    st.title(':violet[PhonePe Pulse EDA Dashborad:signal_strength:]')



c1,c2=st.columns(2)
with c1:
    st.sidebar.title('Map analysis')
    st.sidebar.write("YEAR ANALYSIS")
...
fig.update_traces(marker=dict(color="red" ,size=0.3))
...
st.write("### **:violet[PhonePe Transation Penetration in India ]**")
...
st.sidebar.title("Transactions analysis")
...
with tab1:
    ...
    st.sidebar.write("STATE ANALYSIS")
...
with tab2:
    st.sidebar.write("DISTRICTS ANALYSIS")
...
with tab4:    
    ...
    # with col1:
    st.write('### :violet[Drastical Increase in Transactions :rocket:]')
...
st.write('# :violet[USERS DATA ANALYSIS ]')
tab1, tab2, tab3, tab4 = st.tabs(["STATE ANALYSIS", "DISTRICT ANALYSIS","YEAR ANALYSIS","OVERALL ANALYSIS"])
...
with tab1:
    ...
    st.sidebar.write('STATE AND USER ANALYSIS')
...
st.title(' :violet[TOP 3 STATES DATA]')
c1,c2=st.columns(2)
with c1:
    Year = st.selectbox(
            'Please select the Year',
